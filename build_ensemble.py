...
def build_ensemble_model(
    ...
    ensemble_auc = roc_auc_score(
        test_y, ensemble_predictions, multi_class="ovr"
    )

    # Calculate ensemble F1 and F2
    ensemble_f1 = f1_score(
        test_y, ensemble_predictions.argmax(axis=1), average="weighted"
    )
    ensemble_f2 = fbeta_score(
        test_y, ensemble_predictions.argmax(axis=1), beta=2, average="weighted"
    )
